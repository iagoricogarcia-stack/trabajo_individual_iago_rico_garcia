...
# ======================================================
# 2. CEREBRO MATEMÁTICO: BELLMAN-FORD EN DOS DIMENSIONES 
# ======================================================
...
# =========================================
# 3. INTERFAZ GRÁFICA INTERACTIVA EN PYGAME 
# =========================================
...
class InterfazGraficaPygame:
    def __init__(self, red, analizador):
        ...
        # Sistema de Vistas del Panel Derecho
        self.vista_actual = "mapa" # "mapa", "estaciones" o "tuberias"

        # Inputs de formulario incorporados
        self.inputs = {
            "Estación Origen": "", "Estación Destino": "", "Caudal (L/s)": "",
            "Nueva Estación": "", "Origen Tubería": "", "Destino Tubería": "",
            "Costo Tubería": "", "Capacidad Tubería": "", "ID Tubería Borrar": ""
        }
        ...
        # Mapeo de coordenadas geométricas de nodos
    ...
    def procesar_calculo(self):
        ...
        if simples[dest]['costo'] == float('inf'):
            self.agregar_log("RUTA 1 (Simple): Físicamente inalcanzable.")
        else:
            self.agregar_log(f"RUTA 1 (Simple): {' -> '.join(simples[dest]['camino'])} (Costo: {simples[dest]['costo']})")
            
        if ciclos[dest]['costo'] != float('inf'):
            self.agregar_log(f"RUTA 2 (Ciclo Negativo 1 vez): {' -> '.join(ciclos[dest]['camino'])} (Costo: {ciclos[dest]['costo']})")
            
        self.vista_actual = "mapa" # Forzar vista al mapa para ver los resultados

    def bucle_principal(self):
        reloj = pygame.time.Clock()
        ejecutando = True

        # Geometría de Inputs de Formulario
        rects_inputs = {
            "Estación Origen": pygame.Rect(30, 50, 130, 30), "Estación Destino": pygame.Rect(180, 50, 130, 30), "Caudal (L/s)": pygame.Rect(330, 50, 110, 30),
            "Nueva Estación": pygame.Rect(30, 140, 200, 30),
            ...
            "ID Tubería Borrar": pygame.Rect(30, 320, 150, 30)
        }

        # Geometría de Botones de Evento
        btn_calcular = pygame.Rect(460, 50, 110, 30)
        btn_add_estacion = pygame.Rect(250, 140, 150, 30)
        btn_add_tuberia = pygame.Rect(460, 230, 110, 30)
        btn_del_tuberia = pygame.Rect(200, 320, 110, 30)
        ...
        while ejecutando:
            ...
            for evento in pygame.event.get():
                if evento.type == pygame.QUIT:
                    ...
                    if btn_calcular.collidepoint(pos_mouse):
                        ...
                        try:
                            id_t = int(self.inputs["ID Tubería Borrar"])
                            borrada = self.red.eliminar_tuberia_por_id(id_t)
                            if borrada:
                                self.agregar_log(f"✅ Tubería ID N°{id_t} eliminada de la base de datos.")
                            else:
                                self.agregar_log(f"❌ Error: ID N°{id_t} fuera de rango.")
                            self.inputs["ID Tubería Borrar"] = ""
                        except ValueError:
                            self.agregar_log("❌ Error: Introduce un ID numérico válido.")
            ...
            self.dibujar_boton(btn_add_estacion, "Añadir Estación", self.ACCENT)
            self.dibujar_boton(btn_add_tuberia, "Añadir Conexión", self.ACCENT)
            self.dibujar_boton(btn_del_tuberia, "Eliminar ID", self.ALERT)
            
            # Dibujar los botones de las vistas
            ...
            if self.vista_actual == "mapa":
                ...
                for i, est in enumerate(self.red.estaciones):
                    ...
                    # Línea separadora sutil estilo Excel
                ...
                # CABECERAS TUBERÍAS POR COORDENADAS ABSOLUTAS
                ...
                for i, tub in enumerate(self.red.tuberias):
                    ...
                    self.pantalla.blit(self.fuente.render("->", True, (189, 195, 199)), (750, 100 + i*26)) # Flecha visual
